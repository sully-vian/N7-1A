...
/* Ouvrir le fichier `nom` dans un descripteur de fichier en assurant la bonne
exécution. Retourne le descripteur de fichier associé. */
...
/* Fermer le fichier de descripteur `desc` en vérifiant la bonne fermeture.
`nom` est utilisé en cas d'erreur. */
...
/* Dupliquer (cf `dup2`) en assurant la bonne exécution. `nom` est utlisé en cas
d'erreur. */
...
/* Exécuter `cmd0` avec `execvp` en s'assurant de la bonne exécution. */
...
/* Créer un tube en s'assurant du succès de l'opération */
...
int main(void) {
    bool fini = false;
    int dernier_lu = -1;  // le descripteur du dernier tube lu
        ...
        if (commande == NULL) {
            ...
            if (commande->err) {
                ...
                /* Pour le moment le programme ne fait qu'afficher les commandes
                   tapées et les affiche à l'écran. Cette partie est à modifier
                   pour considérer l'exécution de ces commandes
                */
                int indexseq = 0;
                char **cmd;
                while ((cmd = commande->seq[indexseq])) {
                    // tant qu'on n'est pas à la fin du pipeline
                    if (cmd[0]) {
                        if (strcmp(cmd[0], "exit") == 0) {
                            ...
                            if (commande->seq[indexseq + 1] != NULL) {
                                // s'il y a une commande suivante, créer un tube
                                safepipe(tube);
                            }
                            ...
                            switch (pid_fils = fork()) {
                                ...
                                case 0: /* code fils */
                                    ...
                                    if (dernier_lu != -1) {
                                        ...
                                        // fixer l'entrée standard à la sortie
                                        // du tube précédent et fermer la sortie
                                        // du tube
                                    ...
                                    if (commande->seq[indexseq + 1] != NULL) {
                                        ...
                                        // fixer la sortie standard à l'entrée
                                        // du tube et fermer l'entrée du tube
                                ...
                                default: /* code père */
                                    ...
                                    if (dernier_lu != -1) {
                                        // si un tube a été ouvert, le fermer
                                        // (le père n'en a pas besoin)
                                        safeclose(dernier_lu, "dernier_lu4");
                                    }
                                    if (commande->seq[indexseq + 1] != NULL) {
                                        // s'il y a une commande suivante,
                                        // fermer la sortie (pas besoin dans le
                                        // père)
                                        safeclose(tube[1], "dernier_lu5");
                                        // sauvegarder la sortie du tube pour la
                                        // prochaine commande
                                        dernier_lu = tube[0];
                                    }

                                    // le père attend la terminaison du fils si
                                    // ce dernier n'est pas en arrière plan
                                    if (commande->backgrounded == NULL) {
                                        /* attendre de recevoir un signal. Quand
                                         réception, le handler sera appelé */
                                        pause();
                                    }
                                    break;
                            }
