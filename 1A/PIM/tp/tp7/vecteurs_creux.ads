...
package Vecteurs_Creux is
   ...
   -- Est-ce que le vecteur V est nul ? (ses coordonnées sont toutes absolument inférieures à Epsilon (critère de précision fourni))
   function Est_Nul
   ...
   function Composante_Recursif
     (V : in T_Vecteur_Creux; Indice : in Integer) return Float with
     Pre => Indice >= 1;
   ...
   procedure Modifier
     (V : in out T_Vecteur_Creux; Indice : in Integer; Valeur : in Float) with
     Pre => Indice >= 1, Post => Composante_Recursif (V, Indice) = Valeur;

   -- Est-ce que deux vecteurs creux sont égaux ? (à Epslon près)
   ...
   function Nombre_Composantes_Non_Nulles
     (V : in T_Vecteur_Creux) return Integer with
