...
procedure Specifier_Et_Tester is
    ...
    function Pgcd (A, B : in Integer) return Integer with
        Pre =>  A > 0 and B > 0,
        Post =>  Pgcd'Result >= 1 -- résultat de Pgcd
    ...
    procedure Utiliser_Pgcd is
        Resultat: Integer;
    begin
        Resultat := Pgcd (0, 10); -- Pabon ! 0 n'est pas strictement positif
        Put ("Pgcd (0, 10) = ");
        Put (Resultat, 1);
        New_Line;
    end Utiliser_Pgcd;
    ...
    procedure Permuter (A, B: in out Integer) with
        Post => A = B'Old and B = A'Old -- anciennes valeurs de A et B (avant exécution)
    is
        Copie_A : Integer;
    begin
        Copie_A := A;
