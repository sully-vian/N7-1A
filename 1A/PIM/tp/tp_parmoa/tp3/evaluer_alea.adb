...
procedure Evaluer_Alea is
    ...
    is
        type T_Tableau is array (1..Borne) of Integer;
        
        package Mon_Alea is
            new Alea (1, Borne);
        use Mon_Alea;
        
        Nombre_Alea: Integer; -- stocke le nombre généré
        Tab: T_Tableau; -- stocke le nombre d'occurence
    ...
    begin
        ...
        for I in 1..Taille loop
            ...
            -- Mise-à-jour tableau
            Tab(Nombre_Alea) := Tab(Nombre_Alea) + 1;
            
            -- Mise-à-jour des Max et Min
            if Tab(Nombre_Alea) > Max then
                Max := Tab(Nombre_Alea);
            elsif Tab(Nombre_Alea) < Min then
                Min := Tab(Nombre_Alea);
            end if;
        end loop;

        -- Calcul de la moyenne à partir des tirages
