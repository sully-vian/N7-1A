...
procedure Puissance is
    ...
    begin
        if Exposant < 0 then
            N := 1.0 / Puissance_Positive_Iteratif(Nombre, -Exposant);
        else
            N := Puissance_Positive_Iteratif(Nombre, Exposant);
        end if;
        return N;
    ...
    begin
        N := 1.0;
        if Exposant < 0 then
            N := 1.0 / Puissance_Recursif(Nombre, -Exposant);
        elsif Exposant > 0 then
            N := Nombre * Puissance_Recursif(Nombre, Exposant-1);
        end if;
        return N;
