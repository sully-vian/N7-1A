...
function J_res = J_residu_C14(beta, donnees)
    ...
    dres_dA0 = -exp(-lambda * Ti);
    dres_dlambda = A0 * Ti .* exp(-lambda * Ti);

    % calcul final de la Jacobienne du residu
    J_res = [dres_dA0, dres_dlambda];
...
function [H_f, res, J_res] = Hess_f_C14(beta, donnees, residu, J_residu)
    ...
    % calcul de H_r(beta)
    H11 = [0 0 0 0 0 0 0]';
    H12 = Ti .* exp(-lambda * Ti);
    H21 = H12;
    H22 = -A0 * Ti .^ 2 .* exp(-lambda * Ti);
    Hr_beta = [H11 H12;
               H21 H22];

    % calcul de R_beta
    R_beta = blkdiag(res, res);

    % calcul final de la Hessienne des moindres carrés du residu
    H_f = R_beta' * Hr_beta + J_res' * J_res;
