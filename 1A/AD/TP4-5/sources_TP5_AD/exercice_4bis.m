...
%--------------------------------------------------------------------------
% Classification avec le SVM optimal
%--------------------------------------------------------------------------
...
load hyperparametres_exercice_4; 

% Calcul du SVM optimal
[X_VS,Y_VS,Alpha_VS,c,code_retour] = estim_param_SVM_noyau_marge(X_app,Y_app,sigma_opt_test,lambda_opt_test); % FONCTION A CODER
% Classification des donnees d'apprentissage
Y_app_pred = classification_SVM_avec_noyau(X_app,sigma_opt_test,X_VS,Y_VS,Alpha_VS,c); % FONCTION A CODER
...
Y_test_pred = classification_SVM_avec_noyau(X_test,sigma_opt_test,X_VS,Y_VS,Alpha_VS,c); % FONCTION A CODER
...
% Grille de classification du SVM entre les 2 classes :
[grille_carac1,grille_carac2] = meshgrid(graduation_carac_1,graduation_carac_2);
X_grille = [grille_carac1(:),grille_carac2(:)];
Y_pred_grille = classification_SVM_avec_noyau(X_grille,sigma_opt_test,X_VS,Y_VS,Alpha_VS,c); % FONCTION A CODER
...
figure('Name','Classification par SVM avec noyau gaussien et marge souple (apprentissage)',...
...
figure('Name','Classification par SVM avec noyau gaussien et marge souple (test)',...
