 ...
 * L'arrête (source, destination) appartient à la liste ssi destination
 * appartient à liste et si prec(destination) == source.
 *
 * Pré-conditions : liste != NULL
 *
 * @param liste [in] liste à parcourir
...
/**
 * distance_noeud_liste : récupère la distance associée au noeud donné dans la
 * liste donnée. Si le noeud n'existe pas dans la liste, retourne `INFINITY`.
 *
 * Pré-conditions : liste != NULL
 ...
 * @return distance associée à noeud dans liste ou INFINITY si noeud n'est pas
 * dans liste
 */
float distance_noeud_liste(const liste_noeud_t *liste, noeud_id_t noeud);

/**
 * precedent_noeud_liste : récupère le noeud précédent associé au noeud donné
 * dans la liste donnée. Si le noeud n'existe pas, retourne `NO_ID`.
 *
 * Pré-conditions : liste != NULL
 * Post-conditions : `!contient_noeud_liste(liste, noeud)` =>
 ...
 * liste donnée. Si le noeud n'est pas dans la liste, il est ajouté.
 *
 * Pré-conditions : liste != NULL
 * Post-conditions :
 ...
 * n'est pas dans la liste, ne fait rien.
 *
 * Pré-conditions : liste != NULL
 * Post-conditions : après `supprimer_noeud_liste(liste, n)` :
 * `!contient_noeud_liste(liste, n)`
 *
 * @param liste [in,out] liste à modifier
