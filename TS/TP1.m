...
Fe = 10 ^ 4; %fréquence d'échantillonnage en Hz
...
%des labels sur les axes et un titre (utilisation dFe/Ne xlabel, ylabel et
%title)
figure
plot(temps, x);
grid
...
ZP = 2 ^ 10; % première puiss de 2 supérieure à N
X_ZP = fft(x, ZP);
...
figure
echelle_frequentielle = (0:N - 1) * Fe / N - Fe / 2;
semilogy(echelle_frequentielle, fftshift(abs(X)), 'b'); %Tracé en bleu : 'b'
hold on
echelle_frequentielle = (0:ZP - 1) * Fe / ZP - Fe / 2;
